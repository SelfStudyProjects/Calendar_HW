...
프로젝트명: 세일즈포스 최종 과제 - 개인 일정 및 비용 관리 시스템
소개
...
설치 및 실행 방법
Salesforce DX 프로젝트 클론 및 환경 설정
필요한 패키지 설치: npm install
Salesforce Org에 메타데이터 배포: sfdx force:source:deploy
테스트 클래스 실행 및 검증
로컬에서 LWC 컴포넌트 개발 및 디버깅
문서
...
```mermaid
flowchart LR
    A[메인 대시보드] --> B[Account 리스트]
    A --> C[Contact 리스트]
    A --> D[Opportunity 리스트]
    A --> E[Personal Activity 달력]
    A --> F[비용 집계 보고서]
    A --> G[사용자 설정]
    E --> H[일정 생성 모달]
```
